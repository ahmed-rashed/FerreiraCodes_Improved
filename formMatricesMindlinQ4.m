...
for iElement=1:N_elements
    ...
    for n=1:size(gaussWeights,1)    % cycle for Gauss point
        ...
        % [B_b] matrix bending
        B_b(1,3:3:end)=N_diff_x_y_cols(:,1).';
        B_b(2,2:3:end)=-N_diff_x_y_cols(:,2).';
        B_b(3,2:3:end)=-N_diff_x_y_cols(:,1).';
        B_b(3,3:3:end)=N_diff_x_y_cols(:,2).';
    ...
    for n=1:size(gaussWeights_reduced,1)    % cycle for Gauss point
        ...
        [N_col,N_diff_xi_eta_cols]=shapeFunctionQ4(xi_reduced_Gauss,eta_reduced_Gauss);
        ...
        B_s(1,1:N_nodesPerElement)       = N_diff_x_y_cols(:,1)';  
        B_s(2,1:N_nodesPerElement)       = N_diff_x_y_cols(:,2)';
